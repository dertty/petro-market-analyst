...
Ответы настоящие и приведены целиком — из полного прогона по golden-датасету
10 августа 2026 года (34 вопроса, `reports/golden_agent_dataset.jsonl`; методика
замеров — [docs/EVAL-AGENT.md](docs/EVAL-AGENT.md)). Взяты короткие ответы, по одному
на сценарий: поиск по отчётам, отчёт плюс веб, свежие данные, вопрос вне профиля.
Правки только оформительские: заголовки понижены на три уровня, чтобы не спорить с
оглавлением README, и убрана служебная реплика цикла приёмки там, где агент начинал
ответ с неё.
...
| Подбор моделей и переход на более сильные | `▰▰▱▱▱▱▱▱▱▱` 20% | слоты (основная, лёгкая, судьи, веб-поиск) подобраны по цене и фактическому счёту — выбор каждого разобран в [.env.example](.env.example); сравнения кандидатов по качеству ответа нет, оно упирается в разрешение набора из первой строки |
