...
Четыре итерации на одном и том же наборе из пяти вопросов — по одному на каждый сценарий
датасета. Сравнивать прогоны допустимо только на одинаковых вопросах, поэтому здесь
пятивопросный срез, а не весь датасет: числа в шапке README (78 % и 92 %) относятся
к полному прогону по 34 вопросам и с этим графиком не совпадают.

<img src="assets/metrics-progress.svg" width="940" alt="Пять панелей по четырём итерациям: соответствие протоколу 44, 27, 72, 92 процента; экспертная оценка 80, 70, 80, 80 процентов; латентность 653, 552, 312, 299 секунд; длина траектории 22,6, 15,6, 13,0, 11,8 раунда; стоимость ответа 0,190, 0,164, 0,142, 0,121 доллара"/>
...
- **Выросло только соблюдение контракта.** Скачок 27 → 72 % на третьей итерации дало одно
...
- **Экспертная оценка стоит на месте:** 80 % на первой итерации и 80 % на последней.
  Содержание ответов не улучшилось — улучшилась дисциплина оформления. Провал до 70 %
  на второй — это ровно один потерянный ответ: его вытеснила реплика цикла приёмки.
- **Латентность, длина траектории и стоимость падают вместе,** потому что это одна
  величина в трёх видах: короче траектория — быстрее и дешевле. Четвёртая итерация шла в
  три потока, часть её латентности — ожидание слота модели, поэтому с однопоточными
  прогонами она по этой метрике строго не сравнима.
