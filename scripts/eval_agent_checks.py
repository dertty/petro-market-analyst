...
def report_citations(text: str) -> List[tuple]:
    ...
    Отбор идёт по форме и только по ней — фильтра по издателю здесь быть не должно.
    Отсеивая ссылки с «чужим» названием, разбор прятал бы от `citation_validity` ровно
    те цитаты, которые она обязана поймать: выдуманные. Проверено на сохранённых
    прогонах — усечённое до `[SberCIB, <дата>, с. N]` название так и уходило в тишину.
    """
    found = []
